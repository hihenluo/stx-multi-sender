...
// Smart contract configuration for Stacks Mainnet
...
function App() {
  ...
  // Synchronize wallet session on component mount
  useEffect(() => {
    if (isConnected()) {
  ...
  // Handle wallet connection and disconnection logic
  ...
  // Process textarea input into a filtered list of valid Stacks addresses
  ...
  // Validation: Contract requires between 5 and 50 addresses per batch
  const isValidCount = addressList.length >= 5 && addressList.length <= 50;

  // Execute the airdrop smart contract call
  const handleAirdrop = async () => {
    ...
    try {
      ...
      const response: any = await request('stx_callContract', {
        ...
        network: 'mainnet',
        // FIX: The latest 'request' method expects the string 'allow' 
        // instead of the PostConditionMode enum object.
      ...
      <main className="max-w-3xl mx-auto px-6 py-12">
        <div className="bg-slate-900 border border-slate-800 rounded-3xl p-8 shadow-2xl text-left">
          ...
          <div className="space-y-6">
            <div>
              <div className="flex justify-between mb-2">
                ...
                <span className={`text-xs font-mono px-2 py-0.5 rounded ${isValidCount ? 'bg-green-500/10 text-green-400' : 'bg-red-500/10 text-red-400'}`}>
                  {addressList.length} / 50 Addresses
                </span>
              </div>
              <textarea
            ...
            <div>
              <label className="block text-xs text-slate-400 uppercase tracking-widest font-bold mb-2 font-mono">Amount Per Wallet (STX)</label>
              <div className="relative">
                ...
                />
                <span className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-500 font-bold font-mono tracking-tighter">STX</span>
              </div>
            </div>
          ...
          {txId && (
            <div className="mt-8 p-4 bg-green-500/10 border border-green-500/20 rounded-xl animate-in fade-in">
              <p className="text-green-400 text-sm font-semibold mb-1 flex items-center gap-2">
                <span>✅</span> Transaction Broadcasted
              </p>
